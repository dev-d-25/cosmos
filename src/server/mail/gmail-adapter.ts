...
/**
 * Fetch message metadata via raw `messages.get?format=metadata` call.
 * Uses repeated `metadataHeaders` query params (the correct Gmail API way)
 * instead of the SDK which joins them with commas.
 */
...
/**
 * Fetch full message body via raw `messages.get?format=full` call.
 */
...
/**
 * Fetch message IDs (and nextPageToken) via raw `messages.list` call.
